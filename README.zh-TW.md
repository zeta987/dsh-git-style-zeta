...
寫成模型看字面就能判定的形式：指名確切的那一行、說明只出現一次、並交代
缺少時該怎麼做。像「只在缺少時加入」這種需要模型自行判讀的條件比較容易
被誤讀——實際示範中就有一次被解讀成要使用者開口才加，結果沒補上 trailer。
請勿在 prompt 文字中放入憑證。只有載入此插件的 DSH profile 會受影響。
